...
const seedExercises = async () => {
    try {
        ...
        const { error: deleteError } = await db_1.supabase.from('exercises').delete().eq('is_custom', false);
        if (deleteError) {
            console.error('[Seeder] Failed to clear old default exercises', deleteError);
            process.exit(1);
        }
        console.log('[Seeder] Cleared old default exercises');
        const { error: insertError } = await db_1.supabase.from('exercises').insert(coreExercises);
        if (insertError) {
