...
exports.getDashboardStats = exports.getAnalyticsHistory = exports.getAIInsights = exports.getMuscleImbalanceReport = void 0;
const db_1 = require("../config/db");
const muscleAnalysis_service_1 = require("../services/muscleAnalysis.service");
const getMuscleImbalanceReport = async (req, res, next) => {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, message: 'User context not found' });
            return;
        }
        const analysis = await muscleAnalysis_service_1.MuscleAnalysisService.orchestrateAnalysis(req.user.id);
        res.status(200).json({
            success: true,
            data: {
...
exports.getMuscleImbalanceReport = getMuscleImbalanceReport;
    ...
    try {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                alerts: analysis.suggestions.filter(s => s.toLowerCase().includes('imbalance') ||
...
const getAnalyticsHistory = async (req, res, next) => {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, message: 'User context not found' });
            return;
        ...
        const { data: history, error } = await db_1.supabase
            .from('muscle_scores')
            .select('*')
...
const getDashboardStats = async (req, res, next) => {
    try {
        ...
        const { data: workouts, error } = await db_1.supabase
            .from('workouts')
